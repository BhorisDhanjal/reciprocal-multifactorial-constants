...
\end{table}\par
We can see from the above tables that both the asymptotic approximation are good since they approximates to the exact solution with low error rates for small values of $k$.
\clearpage
%----------------------------------------------------------------------------------------
%	Section 6
%----------------------------------------------------------------------------------------
...
% In this section we will provide 2 asymptotic expansions and its corresponding computations (similarly to section 5).
% \subsection{Known asymptotics}
% \numberwithin{equation}{subsection}
% In this and the following section we will refer to the closed form formula proved in Theorem 6.1.\par 
% To construct asymptotic approximations for GRMFCs we will use the a known result of the behaviour of the lower incomplete gamma function as follows.
% \begin{lemma}
% $$\gamma (a,x) \rightarrow \Gamma(a) \text{ as } x\rightarrow \infty$$
% \end{lemma}
% \textbf{Proof: }The proof is self evident from the definition of the lower incomplete gamma function as follows.
% \begin{align*}
%     \gamma (a, x) &= \int_0^x t^{a-1} e^{-t} dt
%     \intertext{As $x \rightarrow \infty$}
%     \gamma (a, x)&\rightarrow \int_0^\infty t^{a-1} e^{-t}=\Gamma(a) \QED
% \end{align*}
% \par Further we will use the famous Stirling approximation for the Gamma function stated as follows.
% \begin{lemma}
% $$\Gamma(n)=(n-1)!\sim \sqrt{2\pi}n^{n-1/2}e^{-n}$$
% \end{lemma}
% \subsection{Asymptotic approximation for GRMFCs}
% Using the above lemmas we will proceed to provide an asymptotic approimation for GRMFCs
% \begin{align*}
%     m_x(k)&=1+\frac{e^{x^k/k}}{k}\sum_{r=1}^{k}k^{r/k}\gamma\left ( \frac{r}{k}, \frac{x^k}{k} \right )\\
%     \intertext{Note that as $k \rightarrow \infty$, $x^k/k \rightarrow \infty$}
%     &\sim 1+\frac{e^{x^k/k}}{k}\sum_{r=1}^{k}k^{r/k} \Gamma\left( \frac{x^k}{k} \right) \\
%     &\sim 1+\frac{e^{x^k/k}}{k}\sum_{r=1}^{k}k^{r/k} \sqrt{2\pi}\left(\frac{x^k}{k} \right)^{x^k/k-1/2}e^{-x^k/k}
%     \intertext{\emph{For the sake of brevity we will simplify this expression with Mathematica.}}
%     m_x(k) &\sim 1+\frac{\sqrt{2 \pi } (k-1) \sqrt{\frac{x^k}{k}-1} \left(\frac{x^k}{k}\right)^{\frac{x^k}{k}}}{k-k^{\frac{k-1}{k}}} \QED
% \end{align*}
